...
<div class="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-sky-500/5 via-slate-950/0 to-transparent">
    <div class="w-full max-w-md" data-aos="zoom-in">
        ...
        <div class="glass-auto rounded-3xl p-6 sm:p-8 border border-slate-200/50 dark:border-slate-800 shadow-2xl">
            <h2 class="text-xl font-bold tracking-tight text-center">Registrasi Akun Agensi</h2>
            <p class="text-center text-xs text-slate-500 mt-1">Dapatkan akses ke generator aset dan katalog kemitraan</p>

            @if($errors->any())
            <div class="bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs rounded-xl p-3 mt-4 space-y-1">
            ...
            <div class="flex items-center gap-3 my-5">
                ...
                <span class="text-[11px] font-mono text-slate-500 uppercase">Atau Formulir</span>
                <div class="flex-1 h-px bg-slate-200 dark:bg-slate-800"></div>
            </div>
            ...
            <form method="POST" action="{{ route('register.post') }}" class="space-y-3.5">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1">Kata Sandi (Min 8 Karakter)</label>
                    <input type="password" name="password" required
                           class="w-full bg-slate-900/80 rounded-xl px-3.5 py-2 text-xs outline-none focus:ring-1 focus:ring-sky-400 border border-slate-800">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1">Ulangi Kata Sandi</label>
            ...
            <p class="text-center text-xs text-slate-500 mt-5">
                Sudah memiliki akun? <a href="{{ route('login') }}" class="text-sky-400 font-semibold hover:underline">Masuk di Sini</a>
            </p>
        </div>
    </div>
</div>
